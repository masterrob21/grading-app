        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('academic_years.update', $academicYear->id) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')

                        <div>
                            <label for="year" class="block text-sm font-medium text-gray-700">{{ __('Academic Year') }}</label>
                            <input type="text" name="year" id="year" value="{{ old('year', $academicYear->year) }}" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md focus:border-blue-500 focus:ring-blue-500" placeholder="e.g. 2025/2026" required>
                            @error('year')
