                    ...
                    <div class="max-w-xl mx-auto">
                        <form action="{{ route('marks.update', $mark->id) }}" method="POST" class="space-y-6">
                            ...
                            <div>
                                ...
                                <select name="course_id" id="course_id" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md focus:border-blue-500 focus:ring-blue-500" required>
                                    ...
                                    @foreach($courses as $course)
                                        <option value="{{ $course->id }}" {{ old('course_id', $mark->assessment?->course_id) == $course->id ? 'selected' : '' }}>{{ $course->course_code }} - {{ $course->title }}</option>
                                    @endforeach
                                </select>
                                @error('course_id')
                            ...
                            <div>
                                ...
                                <select name="assessment_id" id="assessment_id" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md focus:border-blue-500 focus:ring-blue-500" required>
                                    ...
                                    @foreach($assessments as $assessment)
                                        <option value="{{ $assessment->id }}" data-course-id="{{ $assessment->course_id }}" {{ old('assessment_id', $mark->assessment_id) == $assessment->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <input type="text" name="student_id" id="student_id" value="{{ old('student_id', $mark->enrollment?->student?->student_id) }}" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md focus:border-blue-500 focus:ring-blue-500" required>
                                <div id="student_name" class="mt-2 text-sm text-gray-600">{{ $mark->enrollment?->student?->full_name ? 'Student: ' . $mark->enrollment->student->full_name : '' }}</div>
                                @error('student_id')
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" min="0" max="999.99" name="score" id="score" value="{{ old('score', $mark->score) }}" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md focus:border-blue-500 focus:ring-blue-500" required>
                                @error('score')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <label class="flex items-center gap-3">
                                <input type="checkbox" name="is_locked" value="1" {{ old('is_locked', $mark->is_locked) ? 'checked' : '' }} class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                            ...
                            <div class="flex items-center justify-end gap-4">
                                ...
                                <button type="submit" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 transition">{{ __('Update Mark') }}</button>
                            </div>
                        </form>
                    </div>
                </div>
